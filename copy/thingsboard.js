...
function initializeTranslateTo(editor) {
    editor.translate_to = function(x, y, zoom) {
        ...
        this.zoom = .75; // Reset zoom to 1 for translation
        this.precanvas.style.transform = "translate(" + this.canvas_x + "px, " + this.canvas_y + "px) scale(" + storedZoom + ")";
        this.zoom = storedZoom; // Restore the original zoom level
...
function resetToTopLeft(editor) {
    ...
    editor.translate_to(-minX + marginX, -minY + marginY, editor.zoom);

    console.log('Reset to top-left:', { x: -minX + marginX, y: -minY + marginY });
    
    return editor;
}

function gridMap(editor,id, gridSize = 25, gap = 50) {
...
function init() {
    ...
    // Using jQuery to select all elements with the 'copyNode' class
    ...
    function initializeClipboardHandlers() {
        ...
        drawflowDiv.addEventListener('mouseleave', function() {
            // Stop all dragging when mouse leaves the canvas
            nodeDragging = false;
        ...
        drawflowDiv.addEventListener('mousedown', function(e) {
        ...
        drawflowDiv.addEventListener('mousemove', function(e) {
        ...
        // Keep the keyboard event handlers
        drawflowDiv.addEventListener('keydown', function(event) {
...
console.log("new node data:",newNodeData.data)
        self.ctx.ngZone.run(() => {
            console.log("Attempting to add node to DrawFlow...");
            ...
            $scope.addNodeToDrawFlow(
                ...
                newNodeData.data?.min || null,
                newNodeData.data?.max || null,
                newNodeData.data?.learning_rate || null,
                newNodeData.data
            );
...
// 			data.allWorkers = $scope.allWorkers;
  ...
  <mat-checkbox *ngIf="processName == 'style_part_process'" [(ngModel)]="isMiddleFeedingProcess">Middle Feeding Process</mat-checkbox> 
	    ...
	    $scope.process_list.forEach((process,i) =>{
	        if(process.station_type == 'Feeding'){
                ...
                for(let j=0; j<process.lay_mp; j++){
	                ...
	                $scope.addNodeToDrawFlow('start_process', position_X, position_Y, process.name, process.feeding_parts, process.min_item_completion_time, process.max_item_completion_time, process.learning_rate)
                }
	            ...
	            $scope.addNodeToDrawFlow('style_part_process', position_X, position_Y, process.name, process.feeding_parts, process.min_item_completion_time, process.max_item_completion_time, process.learning_rate)
                }
	        }else{
                position_X = position_X + 350
                    lastHighestParallelProcessCount = process.lay_mp > lastHighestParallelProcessCount ? process.lay_mp : lastHighestParallelProcessCount;
                 for(let j=0; j<process.lay_mp; j++){
                    ...
                    if(process.station_type == 'QC'){
                        $scope.addNodeToDrawFlow('iron_qc', position_X, position_Y, process.name, process.feeding_parts, process.min_item_completion_time, process.max_item_completion_time, process.learning_rate)
                    }else if(process.station_type == 'Intermediate QC'){
                        $scope.addNodeToDrawFlow('intermediate_qc', position_X, position_Y, process.name, process.feeding_parts, process.min_item_completion_time, process.max_item_completion_time, process.learning_rate)
                    }else{
                        $scope.addNodeToDrawFlow('middle_process', position_X, position_Y, process.name, process.feeding_parts, process.min_item_completion_time, process.max_item_completion_time, process.learning_rate)
                    }
    	    ...
    	    if(pesAttributeObservers.length){
    	       rxjs.combineLatest(pesAttributeObservers).subscribe(result => {
    	            ...
    	            rxjs.combineLatest(relationObservers).subscribe(result => {
    	                result.forEach(key => {
                    		saveCalculatedTargetOnPES(flowData[key].data.PES_Id, flowData[key].data.min, flowData[key].data.max, flowData[key].data.learning_rate);
    	                })
	...
	let saveCalculatedTargetOnPES = function(entityId, minTs, maxTs, learningRate){
	   let target = calculatePESTarget(minTs, maxTs, learningRate)
	   if(entityId){
	...
	$scope.setProcessOnCurrentNode = function(process) {
		...
		if(process !=""){
		    $("#node-" + $scope.selectedNodeId).find("#ProcessNameText").html(process.name);
        	$("#node-" + $scope.selectedNodeId).find(".minmaxbox-latest").css("display", "flex");
    	    $("#node-" + $scope.selectedNodeId).find("#maxVal").attr('value',process.max_item_completion_time);
    		$("#node-" + $scope.selectedNodeId).find("#maxVal").val(process.max_item_completion_time);
    		$("#node-" + $scope.selectedNodeId).find("#minVal").val(process.min_item_completion_time);
    		$("#node-" + $scope.selectedNodeId).find("#minVal").attr('value', process.min_item_completion_time);
    		$("#node-" + $scope.selectedNodeId).find("#learningRate").val(process.learning_rate);
    		$("#node-" + $scope.selectedNodeId).find("#learningRate").attr('value', process.learning_rate);
    		currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.ProcessName = process.name;
		    currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.max = process.max_item_completion_time;
		    currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.min = process.min_item_completion_time;
		    currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.learning_rate = process.learning_rate;
		}else{
	    	$("#node-" + $scope.selectedNodeId).find("#ProcessNameText").html('Process Name');
	     	$("#node-" + $scope.selectedNodeId).find(".minmaxbox-latest").css("display", "none");
		    $("#node-" + $scope.selectedNodeId).find("#maxVal").attr('value', "");
			$("#node-" + $scope.selectedNodeId).find("#maxVal").val("");
			$("#node-" + $scope.selectedNodeId).find("#minVal").val("");
			$("#node-" + $scope.selectedNodeId).find("#minVal").attr('value', "");
			$("#node-" + $scope.selectedNodeId).find("#learningRate").val("");
			$("#node-" + $scope.selectedNodeId).find("#learningRate").attr('value', "");
			currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.ProcessName = "";
		    currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.max = "";
		    currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.min = "";
		    currentFlow.drawflow.Home.data[$scope.selectedNodeId].data.learning_rate = "";
		}
...
// 		};
	...
	function addProcessModalController(instance) {
        ...
        /**
         * Function to handle the selection of a process from the dropdown menu.
         * @param {object} event - The event object from the dropdown.
         * @description
         * This function is called whenever a process is selected from the dropdown menu.
         * It will clear the current selected process and add the newly selected process
         * to the selectedProcess array.
         */
		vm.selected = function(event) {
...
// let html = `<h1>Connection modal</h1>`
	...
	function setDragEventsListeners(){
		...
		$scope.addNodeToDrawFlow = function(name, pos_x, pos_y, processName=null, feedingParts=null, minVal=null, maxVal=null, learningRate=null,data=null) {
		    console.log("check processName")
			...
			switch(name) {
				case "start_process":
					...
					let start_process = `
                          <div>
                            ...
                            <div class="minmaxbox-latest">
                                <div style="width:90px;">
                                    <div>LR:(%)</div><input style="width:60px;"  type="text" id="learningRate" onchange="setValue('learningRate')" ${learningRate ? 'value='+ learningRate: ''} name="fname">
                                </div>
                                <div style="width:90px;">
                                    <div>MinTs:(s)</div><input style="width:60px;"  type="text" id="minVal" onchange="setValue('minVal')" ${minVal ? 'value='+ minVal: ''} name="fname">
                                </div>
                                <div style="width:90px;"> 
                                    <div>MaxTs:(s)</div><input style="width:60px;"  type="text" id="maxVal" onchange="setValue('maxVal')" ${maxVal ? 'value='+ maxVal: ''} name="fname">
                                </div>
                            </div>
                            <div class="pes_id">
                        ...
                        const startNodeData = data || {
                                    max: maxVal ? maxVal : 0,
                                    min: minVal ? minVal : 0,
                                    learning_rate: learningRate ? learningRate : 0,
                                    ETS_List: [],
				...
				case "qc_process":
                            ...
                            <div>
                                ...
                                <div class="minmaxbox-latest">
                                    <div style="width:90px;">
                                    <div>LR:(%)</div><input style="width:60px;"  type="text" id="learningRate" onchange="setValue('learningRate')" name="fname">
                                </div>
                                <div style="width:90px;">
                                    <div>MinTs:(s)</div><input style="width:60px;"  type="text" id="minVal" onchange="setValue('minVal')" name="fname">
                                </div>
                                    <div style="width:90px;">
                                        <div>MaxTs:(s)</div><input style="width:60px;"  type="text" id="maxVal" onchange="setValue('maxVal')" name="fname">
                                    </div>
                                </div>
    ...
    setValue = function(inputType){
	    if(inputType == 'minVal'){
	       $("#node-" + $scope.selectedNodeId).find("#minVal").val(event.target.value);
	       $("#node-" + $scope.selectedNodeId).find("#minVal").attr('value', event.target.value);
	    }else if(inputType == 'maxVal'){
	       $("#node-" + $scope.selectedNodeId).find("#maxVal").val(event.target.value);
	       $("#node-" + $scope.selectedNodeId).find("#maxVal").attr('value', event.target.value);
	    }else{
	       $("#node-" + $scope.selectedNodeId).find("#learningRate").val(event.target.value);
	       $("#node-" + $scope.selectedNodeId).find("#learningRate").attr('value', event.target.value);
	    }
